...
def ip_encoder(df, column_name):
    ...
    df_expanded = pd.DataFrame(
        df[column_name].apply(encode_ip).tolist(),
        index=df.index,
...
def number_normalizer(df, column_name):
    ...
    tensor = torch.tensor(df[f"{column_name}_int"].values.astype(float))
    min_val = torch.min(tensor)
    max_val = torch.max(tensor)

    # Avoid division by zero if all values are the same
    if min_val == max_val:
        normalized_tensor = tensor * 0.0
    else:
        normalized_tensor = (tensor - min_val) / (max_val - min_val)

    normalized_series = pd.Series(normalized_tensor.numpy(), index=df.index)
